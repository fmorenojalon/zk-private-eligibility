...
| --- | --- | --- | --- | --- |
| `circuit_p1only` | 12,875 | 4 | 209,067 | — (never presents against a real offering) |
| `circuit_condab` | 13,029 | 4 | 209,067 | — (never presents against a real offering) |
| `circuit_full` | 34,954 | 6 | 222,361 | 302,755 |

Constraint counts reflect `credential-protocol.md §7`'s corrected nullifier (`Poseidon(holder_secret, scope)`, one input fewer than an earlier version) — 88 fewer constraints per configuration than an earlier measurement, uniformly, since all three compute the nullifier the same way. `verifyProof` gas is unaffected either way, exactly as this file's own prediction says: it tracks public-signal count, not constraint count, and neither changed.

Deployment costs, for reference: `Groth16VerifierP1Only` 439,333 gas (1,814 bytes runtime), `Groth16VerifierCondAB` 439,321 gas (1,814 bytes), `Groth16VerifierFull` 479,498 gas (2,000 bytes), `OfferingPolicy` 1,214,667 gas (6,217 bytes) — all one-time costs, paid once at deploy, not per presentation. (Deployment costs shift by a few hundred gas per verifier from an earlier measurement — the new trusted setup's verification-key constants have a different mix of zero/non-zero bytes, which changes init-code deployment cost slightly; `OfferingPolicy`'s own bytecode is untouched by this circuit change, so its deployment cost is exactly unchanged.)
...
- **The `presentEligibility` − `verifyProof` delta is 80,394 gas — larger than the spec's back-of-envelope estimate, and the breakdown explains why.** The prediction named `NullifierRegistry`'s `SSTORE` (~20k for a zero→nonzero write) plus a handful of cold `SLOAD`s as the dominant cost. Both are real contributors, but there's a third the original estimate didn't itemize: five separate cross-contract calls into `EligibilityRegistry` (`sanctionsRoot()`, `currentEpoch()`, `validSetRootAt()`) and `NullifierRegistry` (`consumeIfUnused`), each paying Solidity's external-call overhead (`CALL` opcode, ~2,600 gas base plus argument/return-data handling) on top of the state read itself — five external calls is a meaningfully larger fixed cost than the single `SSTORE` the spec's prediction focused on. This is a real, measured number now (80,394), not the loose estimate the spec stated before measuring; it isn't broken down further here since doing so would need per-opcode tracing, out of scope for this pass.
...
| Network | Gas price | `verifyProof` (222,361 gas) | `presentEligibility` (302,755 gas) |
| --- | --- | --- | --- |
...
| Ethereum mainnet — congested (illustrative) | 50 gwei | $29.46 | $40.12 |
...
Reuses Phase 2's already-generated fixture proofs (`circuits/credential/proof_{p1only,condab,full}.json`) via their Solidity calldata, hardcoded into `test/GasMeasurement.t.sol` — regenerate that calldata with `snarkjs zkey export soliditycalldata public_<name>.json proof_<name>.json` from `circuits/credential/` if those fixtures are ever regenerated (e.g. after a `SCOPE` change or a circuit change to the nullifier construction, both of which have already happened once this phase).
