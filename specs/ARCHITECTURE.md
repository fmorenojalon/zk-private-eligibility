...
| --- | --- | --- | --- |
| Circuits | Predicate logic, compiled to R1CS | circom 2.2.3 | **Built** — 5 circuits (Phase 1 baselines) |
| Local chain | EVM dev chain hosting the verifier | Anvil (Foundry) | **Built** — deploy/verify flow proven |
| On-chain verifier | Groth16 proof verification | Solidity, generated by snarkjs | **Built** — `Groth16Verifier.sol` |
| Holder app | Credential storage, on-device proving | SwiftUI + mopro (Rust/UniFFI) | **Partial** — proving harness built (F1.1), full holder UX (credential storage, consent, QR) is Phase 4 |
| Measurement | Structured performance records | Node (collector) + Python stdlib (analysis) | **Built** — F1.3 |
| Issuer service | Issuance, valid-set tree, revocation, root publication | Node.js + TypeScript (planned) | **Not started** — Phase 3 |
| Platform backend | Offering policy, presentation requests, proof intake | Node.js + TypeScript (planned) | **Not started** — Phase 3/4 |
| Platform frontend | Offering UI, QR presentation, result display | React + Vite (planned) | **Not started** — Phase 4 |
