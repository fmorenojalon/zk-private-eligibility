...
Every public input listed in §5.1 (`validSetRoot`, `jurisdictionRoot`, `sanctionsRoot`, `currentEpoch`, `scope`) is trustworthy for two separate reasons, not one — conflating them is easy to do and worth being precise about.
...
- **Rotation trigger:** any revocation forces a new root and epoch (issuer removes the leaf, recomputes the tree, publishes). Exact rotation cadence policy (e.g. whether epochs also rotate on a fixed timer independent of revocation) is an issuer-service configuration decision, deferred to Phase 3 (§9).
