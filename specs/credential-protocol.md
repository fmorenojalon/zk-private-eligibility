...
The two-*step* structure itself (`attr_hash`, then `leaf`) is load-bearing, not arity-driven: at issuance the issuer receives the raw attribute values to verify them out-of-band (Flow 1) and independently recomputes `attr_hash` from those verified values — but per PR-5 the issuer must never learn `holder_secret`. Only the holder, who alone holds the secret, can compute `leaf`. The holder computes it locally and sends the issuer the opaque `leaf` value (a hash, not a secret preimage) for insertion into the tree. Recomputing `attr_hash` alone tells the issuer what the value *should* be — it does not, by itself, confirm that the `leaf` the holder actually submits was built from that value; §4.3 closes that gap.
...
**The gap.** The issuer verifying attributes and independently recomputing `attr_hash` (§4.2) does not, on its own, guarantee anything about the `leaf` the holder actually hands over for insertion. `leaf = Poseidon(holder_secret, attr_hash)`, and the issuer never learns `holder_secret` (PR-5) — so it has no way to check that equation directly. A dishonest holder could verify one set of attributes with the issuer, then compute `leaf` from a *different*, fabricated `attr_hash`, and the issuer would insert it none the wiser. Every later eligibility proof against that leaf would then honestly — and correctly — prove properties of the fabricated values, since the eligibility circuit has no way to know they were never actually verified. This is a holder-side soundness gap, not an issuer-trust question — it exists even when the issuer verifies honestly, and is therefore not excused by L1 (PRD §11), which is about issuer honesty, not this.

**The fix.** At issuance, alongside `leaf`, the holder generates a small proof: *"I know a `holder_secret` such that `Poseidon(holder_secret, attr_hash) = leaf`,"* where `attr_hash` is a public input fixed to the exact value the issuer just independently verified, and `leaf` is the value being submitted for insertion. The issuer verifies this proof before inserting `leaf` into the tree. This is cryptographic assurance that the leaf is bound to the verified data, without the issuer ever learning `holder_secret` — closing the gap without touching PR-5.
...
| Malicious holder | Controls their own device, credential, and circuit inputs | Producing a valid proof without qualifying attributes; replaying a nullifier within a scope; proving membership after revocation and root rotation; submitting a leaf built from unverified/fabricated attributes at issuance (§4.3) |
...
| --- | --- | --- |
| Device → Issuer | attribute values *(issuance only)*, commitment, leaf-binding proof (§4.3) | holder secret |
...
| Submit a leaf built from fabricated attributes different from what the issuer verified | Issuer verifies a leaf-binding proof (§4.3) before inserting the leaf — `Poseidon(holder_secret, attr_hash) = leaf` for the issuer's own verified `attr_hash` | PR-23 |
