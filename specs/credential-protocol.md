...
**Cost:** this is exactly `circuits/poseidon-baseline`'s arity-2 circuit (`Poseidon(holder_secret, attr_hash)`), already built and measured in Phase 1 (517 constraints, sub-second desktop proving — `BASELINE_RESULTS.md`), with the output forced equal to the public `leaf` instead of exposed freely. No new cryptographic construction — see `circuits/credential/leaf_binding.circom` for the implementation and its own measured results.
...
- `scope`: a field element uniquely identifying the presentation context — in practice, a specific offering (PRD §5's real-estate offering is the running example) — chosen by the platform backend and included in the presentation request (Flow 2, step 2), e.g. `Poseidon(offering_id)`. Groth16 cryptographically prevents a holder from swapping `scope` after proving (§5.4, Layer 1) — but that alone doesn't stop a proof legitimately generated for one offering from being submitted to a *different* offering's access grant. Each offering's own contract must independently check the verified `scope` equals its own canonical identifier before granting access (§5.4, Layer 2) — the same pattern already required for `validSetRoot` (§6), just without which the scope-bound nullifier design offers no real protection at all.
