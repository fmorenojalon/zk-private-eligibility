...
**Why `presentEligibility` checks `publicSignals[epoch] == registry.currentEpoch()` before using it to index `validSetRootAt`.** Since `currentEpoch` is a public input the prover freely chooses at proving time (Groth16 only guarantees the proof is valid for *some* self-consistent set of public inputs, never that they're the *current* ones — `credential-protocol.md §5.4`, cryptographic binding vs. recorded-value cross-checking), a lookup indexed by whatever epoch the *prover's own proof claims* — without first confirming that epoch is actually current — would defeat two things at once: (1) **revocation (P8)** — a revoked holder simply re-presents their original proof, still naming the old epoch, and `validSetRootAt(oldEpoch)` faithfully returns the old root, matching; (2) **expiry (P7)** — an expired holder (`expiry_epoch < currentEpoch`) simply claims an earlier epoch where `expiry_epoch ≥ currentEpoch` was still true, and the in-circuit check (correctly implemented, Phase 2 cases 7/21) faithfully verifies a self-consistent but stale claim. The circuit itself is correct; nothing on-chain binds its `currentEpoch` input to reality without this check. Checking `publicSignals[4]` (the epoch) against `registry.currentEpoch()` directly, *before* using it to index `validSetRootAt`, closes both — the historical mapping stays useful for analysis (§3.1) but the security property comes from currency, not lookup.

**Replay protection (P10) does not depend on this check, deliberately.** If `nullifier` included `epoch` in its preimage, a holder able to name any past epoch could mint a fresh, unconsumed nullifier for the same scope, and this check would be the only thing standing in the way — one freshness check quietly carrying three unrelated security properties instead of two, with epoch changing system-wide on any issuance or revocation (permanently, not just in some narrow window). `nullifier = Poseidon(holder_secret, scope)` (`credential-protocol.md §7`) removes that dependency at the source instead: replay protection holds regardless of epoch, and this check stays needed for P7/P8 only.
...
│   └── mutationQueue.ts        serializes every tree-mutating request through one FIFO queue and
│                                 enforces chain-before-database ordering (§4.4) - no endpoint calls
│                                 chain.ts or attributeStore.ts's write paths directly
├── db/
│   └── schema.sql              §4.2
└── test/
...
| --- | --- | --- | --- |
| `POST /credentials/request` | `{ holderId }` | `{ attrs: {...9 fields}, attrHash }` | Looks up the issuer's own attribute records for this holder (SQLite `holder_attributes`), computes `attr_hash = Poseidon(9 attributes)`, returns both. Nothing is persisted by this call — `attr_hash` is derived, not stored, so it's recomputed fresh wherever it's needed again (§4.2). No verification of anything yet — this is the issuer *disclosing* its own records (Flow 1, step 2). |
| `POST /credentials/submit` | `{ holderId, proof, publicSignals }` | `{ accepted: true, epoch, root }` or `4xx` | Recomputes `attr_hash = Poseidon(9 attributes)` from this holder's *current* `holder_attributes` row and verifies the F2.8 leaf-binding proof against that value and the proof's own `publicSignals`. Reads both `leaf` and `secret_hash` exclusively from `publicSignals` — `publicSignals[1]` and `publicSignals[2]` respectively (`leaf_binding.circom`'s public order is `[attr_hash, leaf, secret_hash]`, verified the same two ways as `circuit_full`'s: the `component main` declaration line, and a value cross-check against `input_leaf_binding.json` vs. `public_leaf_binding.json`). There is no separate `leaf` or `secretHash` request field — nothing exists for the service to insert or compare instead of the verified signals. If `holder_attributes.secret_hash` is `NULL` for this `holder_id` (first issuance), records `publicSignals[2]` there; if it's already set, rejects unless it matches — this is what makes re-issuance with a different `holder_secret` detectable (`credential-protocol.md §4.3.1`). If valid, and this holder already has a non-revoked leaf (`valid_set_leaves WHERE holder_id = ? AND revoked = 0`), marks that leaf revoked as part of the same tree update, then inserts `publicSignals[1]` at a fresh index — one combined mutation, one recomputed root, one `EligibilityRegistry.publishValidSetRoot` call (Flow 1, steps 4–6; `specs/PRD.md` Flow 6, step 3). A holder is never left with two simultaneously-valid leaves. Rejects if the proof doesn't verify, if `attrHash` doesn't match what `holder_attributes` currently implies, or if `secret_hash` doesn't match what's on file. |
| `POST /holders/:holderId/reset-secret-hash` | — | `{ reset: true }` | Sets `holder_attributes.secret_hash` back to `NULL` for this holder — a deliberate, audited issuer action (PR-25), not something a holder can trigger. The next `/credentials/submit` for this `holder_id` is then treated as a first binding: whatever `secret_hash` that proof carries is recorded fresh, with no comparison against the (now-cleared) prior value. Exists specifically for a lost or replaced device (`specs/PRD.md` L13) — without it, losing `holder_secret` would mean permanent exclusion from re-issuance. |
...
| `POST /revoke` | `{ holderId }` | `{ epoch, root }` or `4xx` | Removes the holder's leaf from the valid-set tree (sets it back to the empty/sentinel value at its index), recomputes the root, publishes it — one new epoch (Flow 4, steps 1–2). Rejects if this holder has no non-revoked leaf (never issued, or already revoked) — there is nothing to remove, and publishing a root identical to the current one would mint a real epoch for a non-change. |
...
| `GET /sanctions/path/:identityCommitment` | — | `{ value, nextValue, nextIndex, pathElements, pathIndices, root }` | Returns the low-leaf triple bracketing this `identityCommitment` in the *current* sanctions tree, plus its Merkle path — exactly the private witness P6's non-membership check needs (`credential-protocol.md §5.3`). Needed fresh before every presentation, since `sanctionsRoot` rotates independently of `currentEpoch` and on its own schedule (`credential-protocol.md §6`). Always returns a witness — the sentinel leaves bound the entire field (§5.3), so a bracketing low leaf exists for any `identityCommitment` — and returning one reveals nothing about whether this identity is newly sanctioned, since bracketing structure is public regardless of the holder's own inclusion or absence. |
...
**Why the leaf inserted into the valid-set tree must be `publicSignals[1]`, never a separate request field.** A holder knows their own `holder_secret`, so they can compute `Poseidon(holder_secret, attr_hash_fake)` for any self-chosen `attr_hash_fake` — including one representing attributes the issuer never verified. If `/credentials/submit` accepted `leaf` as its own request field, a holder could submit a genuinely valid F2.8 proof bound to their real, honestly-issued leaf, while separately supplying a *different*, self-fabricated leaf value for insertion. The proof would verify — it's completely real, just for a leaf other than the one that ends up in the tree. That fabricated leaf would later satisfy P8's Merkle-inclusion check for a full eligibility presentation built entirely on fabricated attributes, defeating F2.8/PR-23 entirely despite every individual check passing. Reading `leaf` exclusively from `publicSignals[1]` (§4.1's `/credentials/submit` row) closes this by construction — there's no second value for the service to insert instead of the one the proof actually attests to.
...
CREATE TABLE holder_attributes (
    ...
    secret_hash     TEXT             -- NULL until first issuance; see the note below
);
...
**Re-issuance produces more than one row per `holder_id` — by design, not by accident.** `holder_id` is not the primary key (`tree_index` is), so a holder re-issued a fresh credential legitimately gets a second row at a new index; the old row stays, permanently marked `revoked = 1`. `GET /credentials/:holderId/path` must therefore query `WHERE holder_id = ? AND revoked = 0 ORDER BY tree_index DESC LIMIT 1` — the most recent non-revoked row — rather than assuming one row per holder. This is a query-discipline note, not a schema gap: the schema already permits what re-issuance needs. Marking the old row revoked here happens *automatically*, inside `/credentials/submit` itself (§4.1), the moment a holder who already has a non-revoked leaf submits a new one — it is an unconditional invariant (a holder never has two simultaneously-valid leaves), not a judgment call. That's a different action from the issuer proactively calling `POST /revoke` *ahead of* any re-issuance, which stays a separate, optional action for a holder who shouldn't wait until they next return (`specs/PRD.md` Flow 6, step 2; PR-24).

**`holder_attributes.secret_hash` is what makes re-issuance with a different `holder_secret` detectable.** `NULL` until the first successful `/credentials/submit` for a `holder_id`, then permanently set to that submission's verified `secret_hash` (`publicSignals[2]`) unless an issuer-initiated reset clears it (`POST /holders/:holderId/reset-secret-hash`, §4.1). Every later submission for that same `holder_id` must carry a proof whose own `secret_hash` output matches what's already on file — this is what stops a holder from quietly minting a fresh nullifier space by re-issuing with a new secret (`credential-protocol.md §4.3.1`), independent of anything epoch-related. Keyed on `holder_id` rather than a person-level identifier, because this PoC assumes one device and one `holder_secret` per person (`PRD.md` L13) — `holder_id` is sufficient under that assumption.
...
### 4.4 Mutation Ordering, Failure Handling, and Reconciliation (TR-24, F3.9)

**The gap.** `/credentials/submit`, `/revoke`, `/sanctions/add`, `/jurisdictions/add`, and `/jurisdictions/approve-subset` each write to SQLite *and* send a transaction through `chain.ts`. Nothing so far says what happens if the chain transaction fails after the database write already succeeded — and for `/revoke` specifically, that ordering produces the worst possible outcome: the issuer's own records show the holder revoked, while `EligibilityRegistry` still holds the old root and `presentEligibility` still accepts their proof. A second, independent problem: `server.ts` handles requests over Node's bare `http` module with no framework serializing anything, so two concurrent mutating requests can have their `chain.ts` calls (each an awaited async operation) resolve in an order different from when their database writes happened — publishing roots out of sequence relative to what the database believes happened when.

**The fix — one queue, chain before database.** `mutationQueue.ts` is the only path any endpoint uses to touch a tree: every call to `/credentials/submit`, `/revoke`, `/sanctions/add`, `/jurisdictions/add`, or `/jurisdictions/approve-subset` is pushed onto one FIFO queue, processed one at a time, never concurrently — this alone makes the interleaving problem structurally impossible, regardless of how many HTTP requests arrive at once. Within each queued mutation, the order is **chain first, database second**: the service computes the new root, calls the relevant `chain.ts` function, and *awaits on-chain confirmation* before writing anything to SQLite. If the chain call fails or reverts, the queue entry is rejected and the SQLite write never happens at all — the mutation is a no-op on both sides, not a no-op on one and a commit on the other. This is the reverse of §5's original step order (database, then chain); §5 is corrected below to match.

**Why chain-first, not database-first.** The two orderings fail differently, and only one of them fails safely. Database-first (the original ordering) fails *open*: a chain failure after a successful local write leaves the issuer believing a mutation succeeded — for `/revoke`, that means believing a holder is locked out when the chain still admits them, which is exactly the dangerous direction. Chain-first fails *closed* by comparison: the only way to get a database/chain mismatch is a crash between a *successful* chain confirmation and the follow-up SQLite write, which leaves the database *behind* the chain, not ahead of it — the chain (the thing `presentEligibility` actually checks) is already correct, and only the issuer's own local bookkeeping is stale.

**Reconciling the residual gap.** A crash in that narrow window is still possible, so on every startup the service recomputes its local valid-set tree's root and compares it against `registry.validSetRootAt(registry.currentEpoch())`. A match means the database and chain agree. A mismatch means a mutation's chain call landed but its database write never completed — the service logs this loudly and refuses to serve until an operator reconciles it manually; there is no automatic replay of the missed write. This is a deliberate PoC simplification, not an oversight (L14) — building real crash recovery (a write-ahead log of pending mutations, idempotent replay) is reasonable future work, but the reconciliation *check* itself, which is what actually prevents silently serving from a stale database, is what's specified and required here.
...
1. `POST /revoke` is pushed onto `mutationQueue.ts` (§4.4). The service computes what the tree's root would become with this leaf zeroed, entirely in memory — SQLite is not touched yet.
2. `chain.ts` calls `EligibilityRegistry.publishValidSetRoot(newRoot)` and awaits on-chain confirmation, which increments `currentEpoch` and stores the new root at that epoch. If this call fails or reverts, the queue entry is rejected here and nothing further happens — the leaf remains exactly as it was in SQLite, matching what the chain still holds (§4.4).
3. Only once that transaction is confirmed does the service commit the zeroed leaf to SQLite's `valid_set_leaves` — one new epoch per revocation (and, per §1, per issuance too), keeping "which epoch did this change happen in" unambiguous for later gas/timing analysis.
...
**Re-issuance (`specs/PRD.md` Flow 6) follows the identical sequence, with one difference at step 1.** Instead of `POST /revoke` alone, `POST /credentials/submit` for a holder who already has a non-revoked leaf revokes that leaf *and* inserts the new one in the same call (§4.1, §4.2) — one tree mutation, one recomputed root, one chain confirmation, one epoch, not a revoke-then-insert pair that would otherwise briefly publish a root under which this holder has zero valid leaves for no reason. Both database effects (marking the old row revoked, inserting the new one) commit together, only after that single chain confirmation, per §4.4's chain-first ordering. Steps 2–4 above are otherwise unchanged: one epoch increment, and any presentation still naming the pre-reissuance epoch fails the same epoch-freshness check.
...
**Fixture proofs.** Foundry can't generate a Groth16 proof — every proof the test matrix uses has to already exist. Three cover everything below, all bound to Alice's real Phase 2 credential (same `holder_secret`, same 9 attributes, same tree state):

- `circuits/credential/proof_full.json` / `public_full.json` (Phase 2 output, already exists) — `scope = 0`, `currentEpoch = 5`, i.e. generated assuming it will be presented to "Offering A" at epoch 5. Used by every case in §7.3/§7.4 except case 11b and case 19's second half.
- `proof_full_offeringB.json` / `public_full_offeringB.json` (generated the same way as Phase 2's fixture, `scope` set to `1` instead of `0`, everything else identical). Used only by case 19.
- `proof_full_epoch6.json` / `public_full_epoch6.json` (generated the same way, `currentEpoch` set to `6` instead of `5`, `scope` unchanged at `0`, everything else — including `validSetRoot` — identical, since Alice's own leaf/path never moved). Used only by case 11b, where the whole point is a genuinely independent proof carrying the *same* nullifier as `proof_full.json` despite the epoch difference.
...
| 11b | Alice presents to the same offering successfully, `currentEpoch` then advances (an unrelated holder's issuance or revocation elsewhere — simulated by republishing the identical `validSetRoot`), then a second, genuinely-independent proof for Alice at the new epoch (same `holder_secret`, same `scope`) is presented | reverts at the nullifier check, same as case 11 — case 11 alone resubmits identical calldata at a fixed epoch, so it can't tell whether replay detection survives an epoch advance (caused by *any* holder anywhere, not Alice) the way `nullifier = Poseidon(holder_secret, scope)` (`credential-protocol.md §7`) requires. Both proofs' nullifiers are confirmed identical before submission |
...
| 22 | `POST /credentials/submit` with a leaf-binding proof built against an `attrHash` that doesn't match `Poseidon(9 attributes)` recomputed from this holder's current `holder_attributes` row | rejected — mirrors `leaf_binding.circom`'s own test (Phase 2) but at the service layer, confirming the service actually enforces the check rather than trusting the client |
...
| 25 | `POST /sanctions/add` for a new `identityCommitment`, then `GET /sanctions/path/:identityCommitment` for that exact value | the returned low-leaf triple's `nextValue` now equals the requested value itself — feeding this witness into the non-membership circuit fails the strict `identity_commitment < nextValue` check (§5.3), since this identity is now a genuine member, not bracketed by anything — the dynamic-insertion decision (§1) verified concretely, not just implemented |
| 26 | `POST /sanctions/add` twice, then `GET /sanctions/path/:identityCommitment` for a value strictly between the two newly-added entries | returns a witness bracketing it correctly by the two new entries specifically (not stale neighbors) — confirms the splice procedure maintains adjacency correctly after *multiple* insertions, not just one |
...
| 32 | `POST /credentials/submit` a second time for a holder who already has a non-revoked leaf (re-issuance, `specs/PRD.md` Flow 6), with a new leaf-binding proof built against `holder_attributes` after the issuer updated one of its fields | the prior `valid_set_leaves` row is marked `revoked = 1`, exactly one new row is inserted at a fresh index, exactly one new epoch is published (not two) — `GET /credentials/:id/path` afterward returns only the new leaf's path, and the holder is never left with two simultaneously-valid leaves |
| 33 | `GET /sanctions/path/:identityCommitment` for a value bracketed by two real, previously-added sanctions entries | returns the correct low-leaf triple (`value`, `nextValue`, `nextIndex`) and a path that validates against the current `sanctionsRoot` |
| 34 | `POST /credentials/submit` for a holder who already has a non-revoked leaf, with an invalid leaf-binding proof (or an `attrHash` that doesn't match `holder_attributes`) | rejected (`4xx`); the holder's existing `valid_set_leaves` row is untouched — still `revoked = 0`, same leaf, same index — and no new epoch is published. A rejected re-issuance attempt must never revoke the prior leaf as a side effect of failing to install a new one |
| 35 | `POST /revoke` for a holder with no non-revoked leaf (either never issued a credential, or already revoked by an earlier call) | rejected (`4xx`); `registry.currentEpoch()` is unchanged — no epoch is minted for a non-change |
| 36 | `GET /sanctions/path/:identityCommitment` for a value below the smallest real sanctions entry ever added | returns the `value = 0` sentinel leaf as the low leaf, bracketing correctly at the bottom of the range (`credential-protocol.md §5.3`) |
| 36b | `GET /sanctions/path/:identityCommitment` for a value above the largest real sanctions entry ever added | returns the leaf whose `nextValue` is the `p - 1` sentinel as the low leaf, bracketing correctly at the top of the range (`credential-protocol.md §5.3`) |
| 37 | `POST /credentials/submit` for re-issuance (holder already has a non-revoked leaf and a `secret_hash` on file) with a leaf-binding proof built against a *different* `holder_secret` than the one that produced the stored `secret_hash` | rejected (`4xx`); the existing `valid_set_leaves` row is untouched — still `revoked = 0` — and no new epoch is published, the same "a rejected submission must not touch the prior leaf" property as case 34 |
| 38 | `POST /credentials/submit` for re-issuance with a leaf-binding proof built against the *same* `holder_secret` as the one on file | accepted — same outcome as case 32, now confirmed specifically for the case where `secret_hash` matches, not just `attrHash` |
| 39 | `POST /credentials/submit` where the test harness constructs `publicSignals` with a `secret_hash` that matches the value on file, alongside a leaf-binding proof genuinely built against a *different* `holder_secret` (so the proof itself fails to verify against that `publicSignals` array) | rejected at proof verification, before the `secret_hash` comparison ever runs — confirms the check is the *verified* `publicSignals[2]`, not a value that could be supplied independently of what the proof actually attests to |
| 40 | First-ever `POST /credentials/submit` for a given `holder_id` (`holder_attributes.secret_hash` is `NULL`) | `secret_hash` is recorded as `publicSignals[2]` from the verified proof — confirmed by checking the stored value against an independently-computed `Poseidon(holder_secret)`, not against anything the request body carried |
| 41 | `POST /holders/:holderId/reset-secret-hash`, then `POST /credentials/submit` with a new `holder_secret` | accepted — the reset cleared the prior binding, so this is treated as a first binding, not a mismatch. A second submission after that, with yet another different secret, reverts to being rejected exactly like case 37 |
| 42 | `POST /credentials/submit` with a genuinely valid leaf-binding proof | the row inserted into `valid_set_leaves` has `leaf = publicSignals[1]` exactly — confirmed by independently recomputing `Poseidon(holder_secret, attr_hash)` and comparing against what's actually in the tree, not merely trusting whatever the request happened to carry, since the request shape (`{ holderId, proof, publicSignals }`) has no separate `leaf` field to diverge from it in the first place |
| 43 | `POST /revoke` where the mocked `chain.ts` call is made to fail | the SQLite `valid_set_leaves` row is completely untouched — still non-revoked, unchanged — and no epoch is minted; the mutation is a no-op on both sides, not a partial one (§4.4) |
| 44 | `POST /credentials/submit` where the mocked `chain.ts` call is made to fail | no row is inserted or modified in `valid_set_leaves` or `holder_attributes.secret_hash`, and no epoch is minted — the same all-or-nothing property as case 43, for issuance and re-issuance specifically |
| 45 | Two mutating requests submitted concurrently (e.g. two `/revoke` calls, or a `/revoke` and a `/credentials/submit`), with the first's chain confirmation artificially delayed | the second request's chain call does not begin until the first's has fully resolved and its database write has committed — `mutationQueue.ts` processes them strictly one at a time, never interleaved |
| 46 | Restart the service with the local database deliberately left one mutation behind the chain (simulating the crash window §4.4 describes) | on startup, the service detects `computed local root ≠ registry.validSetRootAt(registry.currentEpoch())`, logs the mismatch, and refuses to serve rather than silently operating on stale state |

### 7.6 What's deliberately not tested here

Gas measurement (§6) is a measurement pass, not a pass/fail test — it has no row in this matrix, the same way Phase 2's constraint counts weren't part of its 43-case test suite either. Platform-backend and holder-app behavior (presentation-request generation, QR flow, consent UI) are Phase 4 scope — this suite tests the contracts and issuer service Phase 3 actually delivers, not the parts of Flow 2 that live in front of them. Unknown/nonexistent `holderId` values across `/credentials/request`, `/credentials/submit`, `/revoke`, and `GET /credentials/:id/path` are likewise not enumerated here — standard HTTP input validation (a 4xx for an ID that doesn't exist), not a cryptographic or protocol property this suite exists to verify.
...
| F3.2 | §3.2 `NullifierRegistry.consumeIfUnused`, tested in §7.2 case 7 and §7.3 cases 11–11b |
...
| F3.4 | §4.1 (`/revoke`, `/sanctions/add`, `/sanctions/path/:identityCommitment`, `/jurisdictions/add`, `/jurisdictions/approve-subset`, `/jurisdictions/:root/path/:jurisdictionCode`), §5 — epoch rotation and tree maintenance, including re-issuance's revoke-and-insert path and its failure mode, tested in §7.5 cases 24–26, 28–36b |
| F3.8 | §4.1 (`/credentials/submit`'s `secret_hash` check, `/holders/:holderId/reset-secret-hash`), §4.2 (`holder_attributes.secret_hash`) — tested in §7.5 cases 37–41 |
| F3.9 | §4.4 (`mutationQueue.ts`'s single-queue serialization, chain-before-database ordering, startup reconciliation) — tested in §7.5 cases 43–46 |
