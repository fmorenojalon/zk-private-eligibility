...
| `POST /credentials/submit` | `{ holderId, leaf, proof, publicSignals }` | `{ accepted: true, epoch, root }` or `4xx` | Recomputes `attr_hash = Poseidon(9 attributes)` from this holder's *current* `holder_attributes` row and verifies the F2.8 leaf-binding proof against that value and the proof's own `leaf`/`secret_hash` public signals — never a cached or client-supplied value for any of the three. Reads `secret_hash` as `publicSignals[2]` (`leaf_binding.circom`'s public order is `[attr_hash, leaf, secret_hash]`) — there is no separate `secretHash` request field, so there's nothing to read by mistake instead of the verified signal. If `holder_attributes.secret_hash` is `NULL` for this `holder_id` (first issuance), records `publicSignals[2]` there; if it's already set, rejects unless it matches — this is what makes re-issuance with a different `holder_secret` detectable (`credential-protocol.md §4.3.1`). If valid, and this holder already has a non-revoked leaf (`valid_set_leaves WHERE holder_id = ? AND revoked = 0`), marks that leaf revoked as part of the same tree update, then inserts the new `leaf` at a fresh index — one combined mutation, one recomputed root, one `EligibilityRegistry.publishValidSetRoot` call (Flow 1, steps 4–6; `specs/PRD.md` Flow 6, step 3). A holder is never left with two simultaneously-valid leaves. Rejects if the proof doesn't verify, if `attrHash` doesn't match what `holder_attributes` currently implies, or if `secret_hash` doesn't match what's on file. |
...
**Re-issuance (`specs/PRD.md` Flow 6) follows the identical sequence, with one difference at step 1.** Instead of `POST /revoke` alone, `POST /credentials/submit` for a holder who already has a non-revoked leaf revokes that leaf *and* inserts the new one in the same call (§4.1, §4.2) — one tree mutation, one recomputed root, one epoch, not a revoke-then-insert pair that would otherwise briefly publish a root under which this holder has zero valid leaves for no reason. Steps 2–4 above are unchanged: one `publishValidSetRoot` call, one epoch increment, and any presentation still naming the pre-reissuance epoch fails the same epoch-freshness check.
