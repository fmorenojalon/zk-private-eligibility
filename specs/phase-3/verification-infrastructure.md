...
| `POST /credentials/submit` | `{ holderId, leaf, proof, publicSignals }` | `{ accepted: true, epoch, root }` or `4xx` | Recomputes `attr_hash = Poseidon(9 attributes)` from this holder's *current* `holder_attributes` row and verifies the F2.8 leaf-binding proof against that value — never a cached one, so there's no window where a stale `attr_hash` from an earlier `/credentials/request` call could still be accepted. If valid, and this holder already has a non-revoked leaf (`valid_set_leaves WHERE holder_id = ? AND revoked = 0`), marks that leaf revoked as part of the same tree update, then inserts the new `leaf` at a fresh index — one combined mutation, one recomputed root, one `EligibilityRegistry.publishValidSetRoot` call (Flow 1, steps 4–6; `specs/PRD.md` Flow 6, step 3). A holder is never left with two simultaneously-valid leaves. Rejects if the proof doesn't verify, or if `attrHash` doesn't match what `holder_attributes` currently implies. |
