...
**Note on p95 at n=3:** a 95th percentile computed from three samples isn't a meaningful tail-latency estimate — treat this column as roughly "the highest of three runs," not a real percentile. It becomes meaningful once more on-device runs accumulate; kept here now for the table's format to stay consistent as that happens, not because it's statistically meaningful yet.
...
- **Arity 10 costs ~2.7× arity 2 in constraints (1,420 vs 517), not the ~3× a naive per-call model would predict** — consistent with Poseidon's fixed per-permutation overhead (full rounds) being shared across more absorbed inputs at higher arity. This is direct evidence that a single Poseidon call for `attr_hash` (arity 10 as measured here and as the schema stood at the time; arity 9 today, after P9's later retirement dropped one attribute) is cheaper than a 5+5-split design would be (2× arity-5 + 1× arity-2 calls, paying that fixed overhead three times instead of once) — the reasoning holds at either arity, only the exact number moved. See [credential-protocol.md §4.2](../specs/credential-protocol.md#42-commitment-structure) for the commitment structure this justifies.
...
- **EdDSA-Poseidon (8,086 constraints) costs roughly the same as a depth-16/20 Merkle proof** — expensive relative to Poseidon/range, but not dominant. This is the primitive [credential-protocol.md §3](../specs/credential-protocol.md#3-issuer-attestation-membership-only-design-decision) deliberately keeps out of the Phase 2 credential circuit's hot path (membership-only attestation instead of in-circuit signature verification) — this baseline number is the cost that decision avoids paying on every proof.
...
- **The iPhone 14 Pro proved poseidon-baseline (arity 2) roughly 12× faster than the desktop path measured here** (36ms vs 432ms) — counter to the common assumption that mobile hardware is slower than desktop. This isn't a real apples-to-apples comparison of hardware, though: the desktop number times `snarkjs`'s WASM-witness-generation-plus-JS-orchestrated proving, while mopro's on-device path uses `rust-witness` (the circuit's wasm transpiled to native code at build time, per [specs/ARCHITECTURE.md](../specs/ARCHITECTURE.md), "Native mobile proving" section) driving a native Rust/arkworks prover — a faster *software* path, running on slower *hardware*, netting out ahead. Re-baselining the desktop side through the same native path (rather than snarkjs) would be needed before drawing conclusions about the phone itself; right now this measures two different implementations, not two devices.
- Estimates are unvalidated until measured against real throughput — the on-device numbers above are that re-baseline for one circuit; the other three still need it once they're wired through mopro.
