...
- **zkey size roughly triples from `circuit_condab` to `circuit_full`** (5.5MB → 16.1MB) despite constraints only ~2.7x'ing — consistent with `BASELINE_RESULTS.md`'s own observation that Groth16 proving-key size scales with constraint count.
- **`leaf_binding.circom` grew from 517 to 932 constraints after adding `secret_hash`** (`credential-protocol.md §7`'s re-issuance fix) — a second, independent Poseidon component (`Poseidon(1)`, binding `secret_hash = Poseidon(holder_secret)`) on top of the original arity-2 `leaf` hash, at 415 extra constraints for one more single-input hash call. That's proportionally larger than the nullifier's own per-input cost noted above (the eligibility circuits lost only 88 constraints for a full input removed from an already-existing Poseidon call) — consistent with this codebase's own repeated observation that each *separate* Poseidon component pays its own full-round overhead, rather than each additional input inside one shared call being close to free. Still the cheapest circuit in this table by a wide margin, and still run once per issuance rather than once per presentation, so its cost is structurally irrelevant next to the eligibility circuits above.
